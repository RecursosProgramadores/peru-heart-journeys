...
import { MapPin, Clock, Download, Ticket } from "lucide-react";
      ...
      <div className="group bg-white rounded-[2rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-border/50 flex flex-col h-full">
        ...
        <div className="p-8 flex flex-col flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-6 border-t border-border/50">
            <Button
              variant="outline"
              className="rounded-xl border-2 hover:bg-muted font-bold text-xs h-11"
              asChild
            >
              <a href={tour.itineraryUrl || "#"} target="_blank" rel="noopener noreferrer">
                <Download size={16} className="mr-2" /> Itinerario
              </a>
            </Button>
            <Button
              className="rounded-xl bg-black hover:bg-black/90 text-white font-bold text-xs h-11 shadow-lg shadow-black/10 transition-all hover:-translate-y-0.5"
